...
export default function CustomSelect({
    ...
    return (
        <div className={`relative ${isOpen ? 'z-50' : 'z-10'} ${className}`} ref={containerRef}>
            ...
            {/* Dropdown Menu */}
            {isOpen && (
                <div className="absolute z-[60] mt-2 w-full min-w-[160px] bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-xl py-1 animate-in fade-in zoom-in duration-200 origin-top">
                    {options.map((option) => (
                        <button
                            key={option.value}
                            type="button"
