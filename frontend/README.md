...
`IBKR_FLEX_XML_V1` 是 `JRN-013` 至 `JRN-015` 的本地文件导入 adapter。后端已有 JRN-013 preview route，但 provider evidence 尚未验证时会在 staging 前返回 `404 FEATURE_DISABLED`。前端当前不得展示该导入入口、Broker Token/Query ID 配置、网络同步按钮或“已连接”状态。
...
- 不要为 Broker Sync、Market、AI/Insights、PDF、risk cards 或 open registration 新增 Beta 导航、设置说明或可用性文案；invite-only onboarding 不属于 open registration。
