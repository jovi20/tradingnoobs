...
| P0 | `IN_PROGRESS` | `JRN-008` Lifecycle 并发、幂等与 legacy projection | 以 `d4e5f6a7b8c9` 为基线，交付 account→position lock、永久 lifecycle 幂等、backdate/sequence 规则、并发 REDUCE/CLOSE 防超量和单事务 legacy projection。 | Active plan M1 |

JRN-000 至 JRN-007 已完成，当前开发任务为 JRN-008；产品整体仍为 `NOT_READY_FOR_PRODUCTION`。source-bound IBKR 实现仍严格等待 JRN-013 至 JRN-015。不得提前做新页面、模型拆分、在线 Broker Sync、Market、AI 或量化功能。
