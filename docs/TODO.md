...
- `JRN-000` 已完成 checkpoint；`JRN-001` 正在 final verification/review，尚未完成。
...
| P0 | `FINAL_VERIFICATION_REVIEW` | `JRN-001` release contract 与 capability ceiling | 冻结币种/标的/事件合同；禁用能力不可由 API、Admin flag、secret 或 job 绕过；稳定 checkpoint 通过最终验证与独立评审。 | Active plan M0 |
