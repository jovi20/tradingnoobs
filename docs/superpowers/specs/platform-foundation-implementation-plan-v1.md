...
| Stage 3：异步一致性与用户读模型基础 | 部分完成 / Bridge landed | Timeline/Lifecycle contract 与首版 API 已落地；D1 统一 job model 与 D2 outbox event 的表、模型、truth outbox write、DB relay 与本地 relay CLI 已开始落地；Timeline 仍由 legacy `Position`/旧 AI 表派生，Redis worker / broader idempotent execution 尚未开始 |
