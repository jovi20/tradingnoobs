...
- `AccountLedgerEntry` 现金真相基础已建立，账户余额 read model 已对有 opening balance 的账户优先走 ledger-derived；正式 opening-balance / dividend / cash adjustment 写入口仍需后续补齐。
...
- FIFO / fee / FX 口径已有 C4 服务起点并接入 legacy truth sync、legacy batch router/import recalculation、positions open-position display、dashboard mark-to-market、account signed market value 与 ledger-derived cash read model；正式 cash adjustment 写入口和部分 analytics/timeline legacy realized PnL 汇总仍未彻底收敛到 truth/ledger-derived。
...
4. 继续完成 `C4`：补正式 opening-balance / cash-adjustment ledger 写入口；随后再把价格/数量/batch 写操作迁往 truth 写路径。
