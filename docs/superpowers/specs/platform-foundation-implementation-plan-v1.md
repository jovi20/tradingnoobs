...
- 已把 `idempotency_service` 接入 truth trade event 与 manual adjustment 写入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位、同一 payload 的重试会返回已完成 lifecycle 响应，不会重复创建 `PositionEvent` / `OutboxEvent` 或重复写 `CASH_ADJUSTMENT` ledger；同 key 不同 payload 返回 409。
