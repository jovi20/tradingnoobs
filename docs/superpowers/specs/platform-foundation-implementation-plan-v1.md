...
| Stage 2：交易真相模型切换 | Bridge landed / C4 部分完成 | `C1` 与 `C2` 已有初版 truth layer 和 legacy sync bridge；`C3` 已有 AccountLedgerEntry 基础 bridge；`C4` 已有 FIFO accounting service 并接入 legacy truth sync、legacy batch router recalculation、positions/dashboard mark-to-market、account signed market value、ledger-derived cash read model、opening-balance ledger write、manual cash-adjustment write、dividend ledger write、首个 truth trade-event write route、最新 active trade-event reversal 与 position-level manual adjustment 后端路由；前端新增/加仓/平仓入口已 truth-first，详情页已守护式暴露最新事件撤销，truth lifecycle 存在时旧批次编辑和整仓删除已降级/保护 |
...
- 已新增 position-level manual adjustment 后端入口 `POST /api/trading-positions/{position_public_id}/adjustments`，会追加 `PositionEvent(MANUAL_ADJUSTMENT)` 与 `AccountLedgerEntry(CASH_ADJUSTMENT)`，进入 lifecycle `ledger_summary.total_adjustments`，但不改 FIFO 数量与 realized PnL。
...
- 仍未完成真正的 hard cutover：后端已有首个 truth trade event 写入口、最新 active event reversal 与 position-level manual adjustment，前端新增/加仓/平仓入口已 truth-first，详情页已守护式暴露最新事件撤销，truth lifecycle 存在时旧批次编辑已降级只读且整仓 legacy 删除受保护；旧 `Position / TradeBatch` 的 MAE/MFE 和部分复盘控件仍是迁移工具，manual adjustment 前端录入仍待补齐。
...
- `AccountLedgerEntry` 现金真相基础已建立，账户余额 read model 已对有 opening balance 的账户优先走 ledger-derived；正式 opening-balance、manual cash adjustment、dividend、首个 realized PnL truth event 写入口、最新 active trade-event reversal 与 position-level manual adjustment 后端路由已落地；前端新增/加仓/平仓已接 truth event，详情页已能撤销最新未撤销的 `ADD / REDUCE / CLOSE`，旧批次编辑在 truth lifecycle 存在时只读，整仓 legacy 删除受保护，非最新历史撤销与 manual adjustment 前端录入仍需后续补齐。
...
3. 继续推进 `C2 + C5` hard cutover：详情页 UI 的叙事字段编辑已接到 truth event 写入口，后端已有首个 trade event 写入口、最新 active event reversal 与 position-level manual adjustment，前端新增/加仓/平仓与最新事件撤销已 truth-first，旧批次编辑在 truth lifecycle 存在时只读且整仓 legacy 删除受保护；下一步处理 manual adjustment 前端录入与非最新历史撤销边界。
...
- Position-level manual adjustment 已有后端路由和回归测试：通过追加 `MANUAL_ADJUSTMENT` 事件与 `CASH_ADJUSTMENT` ledger 记录现金修正，不影响 FIFO 数量与 realized PnL。
- `OPEN` reversal、非最新历史事件撤销、manual adjustment 前端录入与公司行为事件仍待补正式规则和交互。
