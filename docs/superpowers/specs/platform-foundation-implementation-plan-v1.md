...
- 已新增 position-level manual adjustment 后端入口 `POST /api/trading-positions/{position_public_id}/adjustments`，会追加 `PositionEvent(MANUAL_ADJUSTMENT)` 与 `AccountLedgerEntry(CASH_ADJUSTMENT)`，进入 lifecycle `ledger_summary.total_adjustments`，但不改 FIFO 数量与 realized PnL；金额为 0 的 no-op adjustment 会 422 且不写 event/ledger。
...
- 已把 `idempotency_service` 接入 truth trade event 写入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位、同一 payload 的重试会返回已完成 lifecycle 响应，不会重复创建 `PositionEvent` / `OutboxEvent`；同 key 不同 payload 返回 409。
