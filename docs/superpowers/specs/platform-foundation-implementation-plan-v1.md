...
- 仍未完成真正的 hard cutover：后端已有首个 truth trade event 写入口，前端新增/加仓/平仓入口已 truth-first，truth lifecycle 存在时旧批次编辑已降级只读；旧 `Position / TradeBatch` 的删除、MAE/MFE 和部分复盘控件仍是迁移工具。
...
- `AccountLedgerEntry` 现金真相基础已建立，账户余额 read model 已对有 opening balance 的账户优先走 ledger-derived；正式 opening-balance、manual cash adjustment、dividend 与首个 realized PnL truth event 写入口已落地；前端新增/加仓/平仓已接 truth event，旧批次编辑在 truth lifecycle 存在时只读，删除/正式 reversal 语义仍需后续补齐。
...
3. 继续推进 `C2 + C5` hard cutover：详情页 UI 的叙事字段编辑已接到 truth event 写入口，后端已有首个 trade event 写入口，前端新增/加仓/平仓已 truth-first，旧批次编辑在 truth lifecycle 存在时只读；下一步处理删除/撤销/调整的最终 truth 操作模型。
