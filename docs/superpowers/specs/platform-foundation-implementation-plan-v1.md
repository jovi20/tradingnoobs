...
| Stage 3：异步一致性与用户读模型基础 | 部分完成 / Bridge landed | Timeline/Lifecycle contract 与首版 API 已落地；D1 统一 job model 与 D2 outbox event 的表和模型已开始落地；Timeline 仍由 legacy `Position`/旧 AI 表派生，business outbox writes / relay / worker / idempotent execution 尚未开始 |
...
- D1/D2 异步地基已有 schema 起点：`job_definitions`, `job_runs`, `job_run_events`, `idempotency_keys`, `outbox_events` 表与 SQLAlchemy 模型已落地；truth position event 创建路径已开始在同一事务写入 durable outbox rows；outbox relay / worker / job admin UI / idempotent execution 仍未完成，后续 derived refresh 还没有完整异步执行链。
...
- 尚未实现 relay / Redis / worker，也尚未覆盖所有业务事务。
