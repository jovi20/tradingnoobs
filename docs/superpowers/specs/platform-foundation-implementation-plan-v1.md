...
| Stage 2：交易真相模型切换 | Bridge landed / C4 部分完成 | `C1` 与 `C2` 已有初版 truth layer 和 legacy sync bridge；`C3` 已有 AccountLedgerEntry 基础 bridge；`C4` 已有 FIFO accounting service 并接入 legacy truth sync、legacy batch router recalculation、positions/dashboard mark-to-market；旧 `Position / TradeBatch` 仍是主写路径 |
...
- 已新增 `trading_accounting_service` 的 FIFO 口径核心，覆盖 long/short realized PnL、FIFO lot matching、fee netting、remaining open-lot cost basis 与 open-position mark-to-market；legacy truth sync 已改为从 `PositionEvent` 重放结果推导 truth aggregate、event realized PnL 与 ledger amount，不再盲信 legacy realized_pnl；legacy batch router/import recalculation、positions open-position display 与 dashboard open-position aggregation 已改用同一服务重算。
...
- FIFO / fee / FX 口径已有 C4 服务起点并接入 legacy truth sync、legacy batch router/import recalculation、positions open-position display 与 dashboard mark-to-market；account-level market value、账户余额 read model 和部分 analytics/timeline legacy realized PnL 汇总仍未彻底收敛到 truth/ledger-derived。
...
4. 继续完成 `C4`：清理 account-level market value 与账户余额 read model，把它们进一步收敛到 `trading_accounting_service` 与 ledger-derived。
