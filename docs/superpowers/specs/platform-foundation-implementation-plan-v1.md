...
- 已把 `idempotency_service` 接入 truth trade event、dividend 与 manual adjustment 写入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位、同一 payload 的重试会返回已完成 lifecycle 响应，不会重复创建 `PositionEvent` / `OutboxEvent` 或重复写 `DIVIDEND` / `CASH_ADJUSTMENT` ledger；同 key 不同 payload 返回 409。
...
- 已具备 asset/timeframe、broker connection、content source、AI scope 等资源锁与 request idempotency 的通用承载方式；truth trade event、dividend 与 manual adjustment 写入口已完成首批业务级 request idempotency 接入；尚未把导入、手动同步、AI 触发和其他业务入口系统性写入对应 `business_locks` payload / `idempotency_service`，也尚未接入 Redis worker，Timeline Home API 默认也尚未 hard-cut 到 `DerivedTimelineSnapshot` snapshot-only。
