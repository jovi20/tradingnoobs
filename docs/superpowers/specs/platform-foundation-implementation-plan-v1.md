...
- 已把 `idempotency_service` 接入 truth trade event、dividend、manual adjustment、`/api/insights/analyze` 与 `/api/insights/summary/generate` AI 触发入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位、同一 analysis payload 或同一日 summary 生成请求的重试会返回已完成响应，不会重复创建 `PositionEvent` / `OutboxEvent`、重复写 `DIVIDEND` / `CASH_ADJUSTMENT` ledger、重复执行 analytics / LLM 并写入 `AIAnalysisResult`，或因为已生成当日 summary 而把安全重试误报为 400；同 key 不同 payload 返回 409。
