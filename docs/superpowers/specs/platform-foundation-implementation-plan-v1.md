...
- 已把 `idempotency_service` 接入 truth trade event、dividend、manual adjustment 与 `/api/insights/analyze` AI 触发入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位或同一 analysis payload 的重试会返回已完成响应，不会重复创建 `PositionEvent` / `OutboxEvent`、重复写 `DIVIDEND` / `CASH_ADJUSTMENT` ledger，或重复执行 analytics / LLM 并写入 `AIAnalysisResult`；同 key 不同 payload 返回 409。
