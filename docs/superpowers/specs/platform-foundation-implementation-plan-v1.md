...
| Stage 3：异步一致性与用户读模型基础 | 部分完成 / Bridge landed | Timeline/Lifecycle contract 与首版 API 已落地；D1 统一 job model 与 D2 outbox event 的表、模型、truth outbox write、DB relay 与本地 relay CLI 已开始落地；D3 已有最小 job execution state machine 与本地 handler dispatch wrapper；Timeline 仍由 legacy `Position`/旧 AI 表派生，Redis worker / broader idempotent execution 尚未完成 |
...
- 已新增 `trading_accounting_service` 的 FIFO 口径核心，覆盖 long/short realized PnL、FIFO lot matching、fee netting、remaining open-lot cost basis、open-position mark-to-market 与 account signed market value；legacy truth sync 已改为从 `PositionEvent` 重放结果推导 truth aggregate、event realized PnL 与 ledger amount，不再盲信 legacy realized_pnl；legacy batch router/import recalculation、positions open-position display、dashboard open-position aggregation 与 account NAV market value 已改用同一服务重算；账户/仪表盘现金读模型已优先从 `AccountLedgerEntry` 推导，账户创建会为非零 `initial_balance` 写入 `OPENING_BALANCE` ledger entry，账户现金 PATCH 会写入 `MANUAL_CASH_ADJUSTMENT` ledger delta，truth position dividend 写入口会创建 `PositionEvent(DIVIDEND)` 并同步 `AccountLedgerEntry(DIVIDEND)`，且支持 `fx_rate_to_account_ccy` 和可选 `Idempotency-Key` 重放/冲突保护。
- 已新增首个 truth trade event 写入口 `POST /api/trading-positions/{position_public_id}/events`，支持在既有 `TradingPosition` 上追加 `ADD / REDUCE / CLOSE` 价格数量事件；当前回归覆盖 `ADD` 写入、`REDUCE` 写入、`CLOSE` 全量闭仓、partial `CLOSE` 422、closed position 禁止 `ADD`、FIFO replay、truth aggregate 更新与 `AccountLedgerEntry(REALIZED_PNL)` 同步；该写入口已接入可选 `Idempotency-Key`，同 key 同 payload 可重放已完成响应，同 key 不同 payload 会 409 且不重复写事件。
...
- 已新增 position-level manual adjustment 后端入口 `POST /api/trading-positions/{position_public_id}/adjustments`，会追加 `PositionEvent(MANUAL_ADJUSTMENT)` 与 `AccountLedgerEntry(CASH_ADJUSTMENT)`，进入 lifecycle `ledger_summary.total_adjustments`，但不改 FIFO 数量与 realized PnL；金额为 0 的 no-op adjustment 会 422 且不写 event/ledger；该写入口已接入可选 `Idempotency-Key`，避免客户端重试造成现金调整和 ledger 双记。
...
- 前端已新增 `createTradingPositionManualAdjustment` API client 与详情页 cash adjustment 弹窗，可写入 `MANUAL_ADJUSTMENT` truth event 并刷新 lifecycle。
...
- Timeline Home 当前已能把 `DerivedTimelineSnapshot` 混入 timeline event feed；默认仍与 legacy `Position`、`AISummary`、`AIAnalysisResult` 并行，同时已新增尊重 `expires_at`、`actor_targets` 与 `rollout_percentage` 的 `timeline_snapshot_only_enabled` feature flag 作为 snapshot-only 预硬切质量闸，命中后不再触发 legacy quote stale 检查或 LLM config exception builder，但仍不是最终默认 hard-cut truth-only read model。
...
- D1/D2/D3/D4 异步地基已有 schema 起点：`job_definitions`, `job_runs`, `job_run_events`, `idempotency_keys`, `outbox_events`, `business_locks`, `derived_timeline_snapshots` 表与 SQLAlchemy 模型已落地；truth position event 创建路径已开始在同一事务写入 durable outbox rows；DB relay 已能把 pending outbox 转成 queued job run，并可在已有 idempotency key 指向 job run 的 crash-resume 场景下补标 outbox 为 published，且单条 relay 失败会记录 `attempt_count` / `last_error` / 下次可用时间用于排障和重试；truth trade event、dividend、manual adjustment 与 AI analyze / summary generate / weekly report generate 触发入口已接入可选 `Idempotency-Key`，避免用户/客户端重试时重复追加 `ADD / REDUCE / CLOSE` 事件、重复入账股息、双记现金调整、重复生成同一 AI analysis result、重复生成同一日 AI summary 或把同一周报生成重试误报为已存在；truth-derived refresh job 已自动声明 trading-position 级 `business_locks`；通用 `idempotency_service` 已支持 canonical request hash、same-key replay、不同 payload 拒绝、completed response 保存与过期 key 重启；本地 relay CLI 已可手动/cron 触发 DB relay；最小 job execution service 已支持 claim due job、running lock、claim 后清理调度时间、heartbeat、stale running timeout recovery、payload-declared business lock acquire/release、attempt event、handler dispatch、success completion、retry scheduling 与 final failure；本地 DB worker CLI 已可批量消费 due jobs、可选回收超时 running jobs，并提交每个 job；`derived.timeline.refresh` handler 已能读取 truth lifecycle、返回可审计 refresh 摘要，并 upsert 最小 `DerivedTimelineSnapshot`；Timeline Home 已能读取 materialized snapshot 并有 snapshot-only 质量闸，且 snapshot-only 命中时会跳过 legacy 行情 stale 与 LLM config exception 构建；admin job read/action API 与前端 `/admin/jobs` 控制台已可查询 job 列表、详情事件并执行 requeue/cancel；Redis worker / broader idempotent execution 仍未完成，Timeline Home 默认仍未 hard-cut 到 snapshot-only。
- Timeline contract 中的 `cursor` / `limit` 已有 bridge 级实现；最终 truth-backed Timeline read model 已有 `DerivedTimelineSnapshot` 最小落点与读取服务，`/api/timeline/home` 已开始读取并混入 materialized snapshot event，并可通过 `timeline_snapshot_only_enabled` feature flag 隐藏 legacy timeline events 做预硬切验证；默认 hard-cut 到 snapshot-only 仍未开启。
...
3. 继续推进 `C2 + C5` hard cutover：详情页 UI 的叙事字段编辑已接到 truth event 写入口，后端已有首个 trade event 写入口、最新 active event reversal 与 position-level manual adjustment，前端新增/加仓/平仓、最新事件撤销和 cash adjustment 已 truth-first，旧批次编辑在 truth lifecycle 存在时只读且整仓 legacy 删除受保护；下一步处理非最新历史撤销边界。
...
- Position-level manual adjustment 已有后端路由、前端 API/详情页弹窗和回归测试：通过追加 `MANUAL_ADJUSTMENT` 事件与 `CASH_ADJUSTMENT` ledger 记录现金修正，不影响 FIFO 数量与 realized PnL。
- `OPEN` reversal、非最新历史事件撤销与公司行为事件仍待补正式规则和交互。
...
- 已新增 `OutboxEvent` 模型与 `outbox_events` Alembic 迁移。
...
- 已补单条 outbox relay 失败 metadata：失败事件会保留为 `PENDING`、递增 `attempt_count`、写入 `last_error`，并按 retry delay 设置下一次 `available_at`；达到 relay attempt 上限时会进入 `FAILED`，避免后台无法解释的静默失败。
- 已新增本地 relay CLI：`python outbox_relay_cli.py --limit N` 可打开 DB session、执行 relay、提交成功结果，并在异常时 rollback/close。
...
- 已新增 worker heartbeat 与 timeout recovery：`heartbeat_job_run` 会校验 running lock owner 并刷新 `locked_at`；`recover_stale_running_jobs` 会把超时 `RUNNING` job 交给同一 failure/retry 状态机，attempt 未耗尽则进入 `RETRYING`，耗尽则进入 `FAILED`。
- 已新增 `BusinessLock` / `business_locks` foundation 与 `business_lock_service`：同一 `scope/resource_key` 只能有一个 active owner，过期 lock 可被新 owner 接管，释放时必须校验 owner。
- 已把 business lock 接入 `run_next_due_job` 最小执行链：job payload 可声明 `business_locks`，handler 前 acquire，冲突时不执行 handler 并进入 retry，成功/失败后释放已持有锁。
- 已把 truth outbox relay 生成的 `derived.timeline.refresh` job 自动附加 `business_locks=[{scope: "derived.timeline.refresh", resource_key: trading_position_public_id}]`，让同一仓位的派生刷新具备默认互斥。
- 已新增通用 `idempotency_service`：按 `scope/key` 创建 `IdempotencyKey`，用 canonical JSON hash 识别同一请求，重复同请求可 replay，同 key 不同 payload 会拒绝，可保存 completed response，过期 key 可重启为新的 in-progress 请求。
- 已把 `idempotency_service` 接入 truth trade event、dividend、manual adjustment、`/api/insights/analyze`、`/api/insights/summary/generate` 与 `/api/insights/generate` AI 触发入口：客户端传 `Idempotency-Key` 时，同一用户、同一仓位、同一 analysis payload、同一日 summary 或同一 weekly report payload 的重试会返回已完成响应，不会重复创建 `PositionEvent` / `OutboxEvent`、重复写 `DIVIDEND` / `CASH_ADJUSTMENT` ledger、重复执行 analytics / LLM 并写入 `AIAnalysisResult`，也不会因为已生成当日 summary 或同周期 weekly report 而把安全重试误报为 400；同 key 不同 payload 返回 409。
- 已新增本地 DB worker CLI：`python job_worker_cli.py --queue derived --limit N` 可按批次消费 due jobs，每个 job 成功/失败状态转移后提交；`--recover-stale --stale-after-seconds N` 可在消费前回收超时 running jobs；当前回归覆盖 outbox relay 生成 queued job 后由 worker CLI 分派 handler 并标记 `SUCCEEDED` 的闭环。
...
- 已新增 `timeline_snapshot_only_enabled` feature flag 质量闸：开启、未过期且命中 actor target 或稳定 rollout 分桶时 `/api/timeline/home` 仅返回 `DerivedTimelineSnapshot` materialized timeline events，并跳过 legacy quote stale 检查与 LLM config exception builder，用于验证 snapshot-only 读路径；缺省、关闭、过期、未命中 actor target 或未落入 rollout 时仍保持现有混合 feed。
- 已具备 asset/timeframe、broker connection、content source、AI scope 等资源锁与 request idempotency 的通用承载方式；truth trade event、dividend、manual adjustment、AI analyze、summary generate 与 weekly report generate 入口已完成业务级 request idempotency 接入；尚未把导入、手动同步、其他 AI 触发和其他业务入口系统性写入对应 `business_locks` payload / `idempotency_service`，也尚未接入 Redis worker，Timeline Home API 默认也尚未 hard-cut 到 `DerivedTimelineSnapshot` snapshot-only。
...
- 已在 service/worker 层接入 heartbeat 与 stale timeout recovery，并已新增最小 `DerivedTimelineSnapshot` 写入链路、Timeline Home snapshot event 混入读路径与 `timeline_snapshot_only_enabled` snapshot-only 预硬切质量闸；尚未接入 Redis worker、running interrupt/强杀语义与默认 Timeline Home snapshot-only hard cut。
