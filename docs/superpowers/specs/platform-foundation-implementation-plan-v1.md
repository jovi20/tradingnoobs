...
- `AccountLedgerEntry` 现金真相基础已建立，账户余额 read model 已对有 opening balance 的账户优先走 ledger-derived；正式 opening-balance 写入口已落地；dividend / cash adjustment 写入口仍需后续补齐。
...
4. 继续完成 `C4`：补正式 cash-adjustment/dividend ledger 写入口；随后再把价格/数量/batch 写操作迁往 truth 写路径。
