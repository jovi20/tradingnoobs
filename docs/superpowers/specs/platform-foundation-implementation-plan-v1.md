...
- D1/D2 异步地基已有 schema 起点：`job_definitions`, `job_runs`, `job_run_events`, `idempotency_keys`, `outbox_events` 表与 SQLAlchemy 模型已落地；business outbox writes / outbox relay / worker / job admin UI / idempotent execution 仍未完成，后续 derived refresh 还没有稳定异步执行链。
...
- 尚未把业务事务写入 outbox，也未实现 relay / Redis / worker。
