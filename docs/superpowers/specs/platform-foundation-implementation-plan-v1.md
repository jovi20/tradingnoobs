...
- D1/D2/D3/D4 异步地基已有 schema 起点：`job_definitions`, `job_runs`, `job_run_events`, `idempotency_keys`, `outbox_events`, `business_locks` 表与 SQLAlchemy 模型已落地；truth position event 创建路径已开始在同一事务写入 durable outbox rows；DB relay 已能把 pending outbox 转成 queued job run，并可在已有 idempotency key 指向 job run 的 crash-resume 场景下补标 outbox 为 published；truth-derived refresh job 已自动声明 trading-position 级 `business_locks`；通用 `idempotency_service` 已支持 canonical request hash、same-key replay 与不同 payload 拒绝；本地 relay CLI 已可手动/cron 触发 DB relay；最小 job execution service 已支持 claim due job、running lock、heartbeat、stale running timeout recovery、payload-declared business lock acquire/release、attempt event、handler dispatch、success completion、retry scheduling 与 final failure；本地 DB worker CLI 已可批量消费 due jobs、可选回收超时 running jobs，并提交每个 job；`derived.timeline.refresh` bridge handler 已能读取 truth lifecycle 并返回可审计 refresh 摘要；admin job read/action API 与前端 `/admin/jobs` 控制台已可查询 job 列表、详情事件并执行 requeue/cancel；Redis worker / broader idempotent execution 仍未完成，最终 materialized timeline refresh 还没有完整异步执行链。
...
- 已新增 `derived.timeline.refresh` bridge handler：可基于 `trading_position_public_id` 读取 truth lifecycle，返回 handler/source/position/event/node count 等 job result 摘要，作为后续 materialized timeline refresh 的挂点。
- 已具备 asset/timeframe、broker connection、content source、AI scope 等资源锁与 request idempotency 的通用承载方式；尚未把所有业务入口系统性写入对应 `business_locks` payload / `idempotency_service`，也尚未接入 Redis worker 与最终 derived timeline materialization。
...
- 已在 service/worker 层接入 heartbeat 与 stale timeout recovery；尚未接入 Redis worker、running interrupt/强杀语义与最终 materialized timeline refresh。
