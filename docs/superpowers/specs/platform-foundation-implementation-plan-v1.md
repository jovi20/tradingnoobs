...
- 已新增首个 truth trade event 写入口 `POST /api/trading-positions/{position_public_id}/events`，支持在既有 `TradingPosition` 上追加 `ADD / REDUCE / CLOSE` 价格数量事件；当前回归覆盖 `REDUCE` 写入、`CLOSE` 全量闭仓、partial `CLOSE` 422、FIFO replay、truth aggregate 更新与 `AccountLedgerEntry(REALIZED_PNL)` 同步。
...
- 仍未完成真正的 hard cutover：后端已有首个 truth trade event 写入口，但旧 `Position / TradeBatch` 仍是前端价格、数量、批次和部分复盘的主写模型；详情页已有 truth-first 入口，C5 叙事字段已接到 truth event 写入口，但旧批次价格/数量编辑流尚未切换。
...
- FIFO / fee / FX 口径已有 C4 服务起点并接入 legacy truth sync、legacy batch router/import recalculation、positions open-position display、dashboard mark-to-market、account signed market value、ledger-derived cash read model、opening-balance ledger write、manual cash-adjustment write、dividend ledger write 与首个 REDUCE/CLOSE truth write route；ADD 边界、前端价格/数量/批次迁移和部分 analytics/timeline legacy realized PnL 汇总仍未彻底收敛到 truth/ledger-derived。
...
3. 继续推进 `C2 + C5` hard cutover：详情页 UI 的叙事字段编辑已接到 truth event 写入口，后端已有首个 trade event 写入口；下一步补 `ADD` 边界回归，并把前端价格/数量/批次操作从 legacy `Position / TradeBatch` 迁到 `TradingPosition / PositionEvent` 写路径。当前旧控件已明确标成迁移工具，但仍不是最终 truth 写路径。
