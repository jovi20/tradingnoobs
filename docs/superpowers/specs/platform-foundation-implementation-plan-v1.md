...
| Stage 2：交易真相模型切换 | Bridge landed / C4 部分完成 | `C1` 与 `C2` 已有初版 truth layer 和 legacy sync bridge；`C3` 已有 AccountLedgerEntry 基础 bridge；`C4` 已有 FIFO accounting service 并接入 legacy truth sync 与 legacy batch router recalculation；旧 `Position / TradeBatch` 仍是主写路径 |
...
- 已新增 `trading_accounting_service` 的 FIFO 口径核心，覆盖 long/short realized PnL、FIFO lot matching、fee netting 与 remaining open-lot cost basis；legacy truth sync 已改为从 `PositionEvent` 重放结果推导 truth aggregate、event realized PnL 与 ledger amount，不再盲信 legacy realized_pnl；legacy batch router/import recalculation 已改用同一服务重算 `Position / TradeBatch` 聚合。
...
- FIFO / fee / FX 口径已有 C4 服务起点并接入 legacy truth sync 与 legacy batch router/import recalculation；dashboard、open position unrealized 仍有散落的平均成本/请求内计算，尚未彻底中心化。
...
4. 继续完成 `C4`：将 dashboard 与 open position unrealized 的平均成本/请求内计算逐步接入 `trading_accounting_service`，并把账户余额 read model 进一步收敛到 ledger-derived。
