...
## Remaining Recharts Inventory

Current Recharts imports:
...
- [ ] Create `frontend/tests/chart-renderers.test.mts` with a static scan that fails on `from 'recharts'` or `from "recharts"` in `frontend/app`, `frontend/components`, and `frontend/lib`.
- [ ] Temporarily allow the five known files above with an allowlist so the test can land before migration.
- [ ] Run:
...
- [ ] Create `frontend/components/charts/renderers/chartGeometry.ts` with:
...
- [ ] Add tests in `frontend/tests/chart-renderers.test.mts`:
...
- [ ] Create:
  ...
  - `SvgPieChart.tsx`
  - `SvgScatterChart.tsx`
- [ ] Update `frontend/components/insights/LegacyAnalysisChart.tsx` to use `SvgBarChart`.
- [ ] Update `frontend/components/dashboard/workbench/DashboardEquityHero.tsx` to use `SvgLineChart`.
- [ ] Update `frontend/components/dashboard/AllocationPieChart.tsx` to use `SvgPieChart`.
- [ ] Update `frontend/components/dashboard/MaeMfeScatterPlot.tsx` to use `SvgScatterChart`.
- [ ] Keep existing empty-state and `ChartFrame` behavior.
- [ ] Remove these four files from the static allowlist.
- [ ] Run:
...
- [ ] Create `frontend/components/charts/renderers/SvgSankeyChart.tsx`.
- [ ] Use existing `buildPortfolioSankeyChartView(...)` output as input.
- [ ] Render:
...
- [ ] Update `frontend/components/PortfolioSankey.tsx`.
- [ ] Remove the final Recharts static allowlist entry.
- [ ] Run:
...
- [ ] Remove `recharts` from `frontend/package.json`.
- [ ] Run package lock update:
...
- [ ] Authenticated browser smoke covers:
  - `/dashboard` equity line, allocation pie, MAE/MFE scatter, portfolio sankey.
  - `/insights` analysis bar chart.
- [ ] Confirm mobile viewport does not overflow chart cards.
- [ ] Update `docs/TODO.md` with P18 completion and P19 next lane.
