...
- Platform config runtime now has a shared `get_feature_flag_enabled` service helper for enabled/expired/actor-target/stable-rollout flag resolution; Timeline Home uses this instead of a router-local flag check.
