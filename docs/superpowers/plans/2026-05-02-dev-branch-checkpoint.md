...
- Timeline Home now has a pre-hard-cut `timeline_snapshot_only_enabled` feature flag: when enabled, unexpired, and targeted to the current user when `actor_targets` are present, `/api/timeline/home` returns only `DerivedTimelineSnapshot` materialized timeline events and hides legacy position/AI/system timeline events; absence/disabled/expired/untargeted keeps the default mixed bridge feed.
- Platform config runtime now has a shared `get_feature_flag_enabled` service helper for enabled/expired/actor-target flag resolution; Timeline Home uses this instead of a router-local flag check.
