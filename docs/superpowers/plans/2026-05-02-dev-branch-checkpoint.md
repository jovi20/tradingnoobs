...
- D3 minimum job execution service can claim due queued/retrying jobs, lock them to a worker, heartbeat running locks, recover stale running locks into the same retry/final-fail state machine, increment attempt count, record start events, dispatch registered handlers by `JobDefinition.key`, complete jobs successfully, schedule retries, and mark exhausted jobs failed.
...
- D1 unified job model, D2 outbox event schema/relay/CLI, and D3 minimum job execution state machine plus local DB worker CLI/bridge handler have migration/model/service regressions, including relay crash-resume idempotency, outbox-to-worker local dispatch, heartbeat lock refresh, and stale running recovery; Redis worker, final derived materialization, and broader D3 business-lock/idempotent execution rules remain pending.
