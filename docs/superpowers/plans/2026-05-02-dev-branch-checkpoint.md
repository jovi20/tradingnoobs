...
- Timeline Home now has a pre-hard-cut `timeline_snapshot_only_enabled` feature flag: when enabled, `/api/timeline/home` returns only `DerivedTimelineSnapshot` materialized timeline events and hides legacy position/AI/system timeline events; absence/disabled keeps the default mixed bridge feed.
