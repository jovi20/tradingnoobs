...
- field contract 必须显式声明 quantity 是 positive magnitude 还是 signed-by-side，account inception 来自 statement 还是指定 nested element，OpenPositions 日期来自 container 还是逐 position；逐 position 日期必须完整且一致。report-end current OpenPositions 不能冒充 opening flat boundary。若同一 statement 含 contract 未覆盖的 asset/event（包括缺少 exchange/open-close 的 CASH execution），整个 statement fail-closed，禁止过滤后部分导入。
...
- adapter enable gate 必须建立逐语义 provider evidence matrix，而不是用一份普通 fixture 证明全部合同。来自同一文档化 Flex Query 模板的脱敏真实 fixtures/statement pairs 至少分别证明：基础 execution 字段、跨 generation 重叠与 marker 严格顺序/tie、account inception 或完整 OpenPositions boundary、`transactionID/openCloseIndicator`、独立/同 ID cancel-bust/correction target、commission sign/currency。官方字段合同与真实 fixture 缺一不可；官方合同必须绑定允许的 IBKR 官方 URL、取得日期、留存 UTF-8 artifact、SHA-256、locator 与逐字引用，不能只声明 semantic 标签。`VERIFIED` field contract 消费的全部 provider wire element/attribute 名和枚举值还必须作为 `wire_tokens` 逐字存在于绑定的官方摘录；枚举必须绑定字段和值片段（例如 `openCloseIndicator="O"`），不能用文档其他位置的孤立 `O` 代替。field contract 必须显式选择 element-name 或 attribute-value event discriminator，冻结 ordinary/correction/cancel、side、open-close、commission charge sign 与 commission/trade currency 关系，并声明 generation/execution/statement date 的 source-timezone 语义；execution status 必须显式选择 provider attribute 或由 event kind 派生，change identity 必须显式选择独立 event/target ID 或 event ID 即 target，未消费的 placeholder 字段不得继续成为必需 wire token。真实 fixture 的 commission 必须是 finite non-zero 且符合该 sign/currency 合同；flat boundary 必须满足 `fromDate <= accountInceptionDate`，或恰好一个位于 `fromDate`、账户一致且所有 position quantity 均为 finite zero 的完整 OpenPositions snapshot，字段出现本身不能算证明。coverage fixture 必须包含 statement coverage 两端的实际本地日期成交；correction/cancel fixture 必须同时覆盖两类 change、符合冻结的 source/target identity 关系，且 target 能链接到同一 evidence set 的真实 trade execution。语义标签、展示层字段名或真实 fixture 单独均不能证明 raw XML 合同。当前 synthetic tests 使用的 `TradeCorrection/TradeCancel`、`sourceEventID/affectedIBExecID`、`tradeStatus` 与 `OPEN/CLOSE` 只是未验证 placeholder，不得据此开放 adapter；公开实现提示真实报告可能采用 `Trade transactionType=...`、original/related ID 与 `O/C`，但非官方资料只能触发风险评审，不能替代 IBKR 官方证据。synthetic fixture 只补恶意输入和边界。任一必需语义无法证明、wire token 缺失、manifest 损坏或 artifact/hash/quote 不匹配时 `IBKR_FLEX_XML_V1` 保持 `FEATURE_DISABLED`，JRN-013 不得关闭。
...
- 2026-07-26 JRN-013 进度 checkpoint `9a7b4ab` 增加 evidence-first 本地文件 route、持久 conflict session read 与严格官方 evidence artifact/hash/locator/quote gate；默认 manifest 仍为 `UNVERIFIED`，route 在 staging 前返回 `404 FEATURE_DISABLED`，不改变任务退出门或 JRN-014/015 依赖。
- 2026-07-26 JRN-013 checkpoint `fc69c03` 固定 generation 为 UTC instant 升序，并将同 binding、同 marker、不同文件稳定归为 `SOURCE_GENERATION_CONFLICT`；同时补齐 5,000/5,001 execution、XML 节点/属性、generation/coverage/timezone 与 DST fold/gap 边界。默认 evidence 仍未验证，因此任务继续阻断且不得进入 JRN-014。
- 2026-07-26 JRN-013 checkpoint `0f312b6` 补齐同文件跨 upload key 重传不新增 statement/sighting，并将同 statement、同 event ID、不同 fingerprint 的 TRADE/change rows 在 order/target 分支前统一归为 `SOURCE_PAYLOAD_CONFLICT`；每个 observation/sighting 与冲突 episode 均永久保留。完整 gate 为 PostgreSQL 16.14、后端 642、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿，但外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `3ddbb0d` 补齐 accepted correction/cancel-bust exact replay、strict-earlier stale、same-identity payload conflict、OPEN/DIVERGED strict-later episode 收口与重开，以及 fingerprint payload/version migration 的 fail-closed 验收。全部 JRN-013 测试 107 项，完整 gate 为 PostgreSQL 16.14、后端 648、前端 165 及其余合同/构建全绿；真实 provider evidence 与独立 review 退出门不变。
- 2026-07-26 JRN-013 checkpoint `450ced4` 将 external account/event/target/transaction、conid/status/currency 与 source order key 的 parser 上限对齐永久 PostgreSQL 列宽，避免 preview 后才由数据库截断/报错；同时验收同 owner 两个 CLEAN 账户可并行 preview 同一 external account 且零 binding、未知资产 session-only conflict、side/open-close 和超长 transaction key fail-closed。全部 JRN-013 测试 114 项，完整 gate 为 PostgreSQL 16.14、后端 655、前端 165 及其余合同/构建全绿；外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `cbabe32` 固定 terminal normalized preview rows 在第 30 天前一微秒不可清理、恰好第 30 天才可清理，并证明 cleanup 后 ImportSession、idempotency、binding、statement、observation 与 sighting 永久保留；OpenAPI 同时锁定 IBKR confirm/rebind/transfer/binding mutation 路由不存在。全部 JRN-013 测试仍为 114 项，完整 gate 为 PostgreSQL 16.14、后端 655、前端 165 及其余合同/构建全绿；外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `33b5131` 逐表验证跨 owner 改写 binding、observation、sighting、execution 与 reconciliation case 均由数据库复合外键拒绝，补齐 source graph 的纵深 tenant boundary；公开 ImportSession ID 的 owner-first 404 合同沿用既有路由测试。全部 JRN-013 测试 115 项，完整 gate 为 PostgreSQL 16.14、后端 656、前端 165 及其余合同/构建全绿；外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `87970b7` 将 bound `source_preview_digest` 升级为 schema version 2，并把每个 accepted execution 的 disposition、current/canceled observation fingerprint 与 active application state 纳入 canonical payload；即使 authority mutation 漏增 binding revision，旧 preview 也会失效。回归同时证明 revision、current observation、tombstone/canceled-by 与 application group boundary 任一变化都会改变 digest。全部 JRN-013 测试 115 项，完整 gate 为 PostgreSQL 16.14、后端 656、前端 165 及其余合同/构建全绿；外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `1f5ee72` 将 IBKR `symbol` 与 `listingExchange` parser 上限分别收敛为冻结 identity 合同和永久 PostgreSQL schema 的 50/32，超限在 bound preview 写入永久 statement/source truth 前稳定返回 `IBKR_FIELD_TOO_LONG`；精确 50/32 边界仍可解析。全部 JRN-013 测试 115 项，完整 gate 为 PostgreSQL 16.14、后端 656、前端 165 及其余合同/构建全绿；外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `c691edb` 将 owner-scoped account 锁与 404 校验前移到任何上传读取或临时文件 staging 之前；不存在或跨 owner account 均不产生文件 I/O、临时文件、idempotency 或 ImportSession side effect，并始终关闭上传句柄。全部 JRN-013 测试 116 项，完整 gate 为 PostgreSQL 16.14、后端 657、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；默认 provider manifest 仍为 `UNVERIFIED`，外部 evidence 阻断不变。
- 2026-07-26 JRN-013 checkpoint `7bd90fd` 将 IBKR 官方 Trades Flex Reference 作为 UTF-8 artifact、SHA-256、locator 与逐字 field-table quote 纳入 provider manifest，机器 gate 现可证明 `BASIC_EXECUTION_FIELDS` 已有官方证据；测试同时锁定其余六类官方语义仍缺失，不能用字段存在推导 `ibExecID` 稳定性、generation/coverage、change target 或 commission sign。全部 JRN-013 测试 117 项，完整 gate 为 PostgreSQL 16.14、后端 658、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；manifest 继续 `UNVERIFIED`，真实模板/statement pairs 与其余官方合同阻断不变。
- 2026-07-26 JRN-013 checkpoint `0e0f143` 审计发现 synthetic parser 的独立 `TradeCorrection/TradeCancel`、`sourceEventID/affectedIBExecID`、`tradeStatus` 与 `OPEN/CLOSE` wire shape 未获 IBKR 官方合同证明，且可能与真实 Flex XML 不兼容；provider gate 因而新增 exact `wire_tokens` 约束，`VERIFIED` field contract 消费的全部 raw element/attribute 名和枚举值必须逐字存在于 hash-bound 官方摘录，只有 semantic 标签、展示字段表或真实 fixture 不能激活 adapter。新增 P0 provider-contract blocker，不回退既有 fail-closed/source-truth/preview 基础设施。全部 JRN-013 测试 119 项，完整 gate 为 PostgreSQL 16.14、后端 660、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；manifest 继续 `UNVERIFIED`，取得官方 raw-wire 合同与同模板真实 correction/cancel 样本前不得关闭 JRN-013。
- 2026-07-26 JRN-013 checkpoint `515f857` 将 event classifier 与 side/open-close wire 值全部移入显式 field contract：支持 element-name 和 attribute-value 两种策略，禁止隐藏默认、混用策略、缺失或重复 discriminator/enum，并把 provider 值规范到 canonical event/side/open-close。evidence gate 同步按选定策略验证真实 fixture 值，并要求 field-value/element 绑定 token，孤立的 `O/C` 不能证明 `openCloseIndicator="O|C"`。IBKR 官方 Flex Codes 的 `Ca/Co/O/C` 含义作为 hash-bound supporting evidence 入库，但不冒充 correction target 或 raw attribute 证明。全部 JRN-013 测试 131 项，完整 gate 为 PostgreSQL 16.14、后端 672、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；默认 manifest 仍无 field contract 且保持 `UNVERIFIED`，外部 raw-wire/template/真实 statement blocker 不变。
- 2026-07-26 JRN-013 checkpoint `a46887b` 将 commission charge sign 与 commission/trade currency 关系提升为显式 field contract，并要求真实 fixture 以 finite non-zero 值证明该语义；flat-boundary evidence 同步收紧为有效 inception 范围，或位于 coverage start、账户一致且所有 quantity 均为 finite zero 的唯一完整 OpenPositions snapshot，字段存在本身不再算证明。全部 JRN-013 测试 135 项，完整 gate 为 PostgreSQL 16.14、后端 676、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；默认 manifest 仍为 `UNVERIFIED`，真实 provider 合同与同模板 statement pairs 的 blocker 不变。
- 2026-07-26 JRN-013 checkpoint `03632e1` 将 statement/generation/execution 的 source-timezone 语义、execution status 来源与 change identity 模式全部提升为显式 field contract；未由合同消费的 `tradeStatus/affectedIBExecID` 不再成为伪必填 wire token。coverage fixture 现在必须出现两端本地日期成交，correction/cancel evidence 必须在同一 evidence set 覆盖两类 change、符合 same-ID 或 distinct-ID 合同并链接到其中的 trade execution。全部 JRN-013 测试 142 项，完整 gate 为 PostgreSQL 16.14、后端 683、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；默认 manifest 仍为 `UNVERIFIED`，外部 raw-wire/template/真实 statement blocker 不变。
- 2026-07-26 JRN-013 checkpoint `ba490a4` 增加真实 Flex fixture 脱敏工具：draft field contract 驱动跨 statement identity alias，只保留合同消费的 statement 子树和属性，拒绝 DTD/entity/XInclude、namespace、符号链接及超限输入，并以 `0700/0600` exclusive output 和失败清理降低真实账户数据泄漏风险。报告固定为 `NOT_PROVIDER_VERIFICATION` 且必须人工复核，不能自行升级 manifest。全部 JRN-013 测试 152 项，完整 gate 为 PostgreSQL 16.14、后端 693、前端 165 及 OpenAPI/release contract/typecheck/lint/build 全绿；未取得真实模板/fixtures/raw-wire 官方合同前 JRN-013 和 route 仍保持阻断。
- 2026-07-26 JRN-013 checkpoint `7743919` 增加只读 provider evidence readiness CLI，开发者可直接对 manifest/evidence root 运行与 route 相同的 artifact/hash/quote/exact-wire-token/fixture semantic gate，并取得稳定 JSON blocker；CLI 不输出 Query Template ID、不修改 manifest，只有全部证据通过才返回 0。全部 JRN-013 测试 155 项，精确 SHA 完整 gate 为 PostgreSQL 16.14、后端 696、前端 165、217 行新增 secret scan 及 OpenAPI/release contract/typecheck/lint/build 全绿；默认仓库报告仍为 `BLOCKED`，JRN-013 外部证据门不变。
- 2026-07-16 当时的代码现实复核确认 legacy `BrokerExecution.idempotency_key` 只在 RAW 层跳重、legacy IBKR parser 有不可信 ID fallback，且尚无 source binding/fingerprint/coverage/canonical application/reconciliation；该历史发现解释了 JRN-013 至 015 的范围，不代表 `9a7b4ab` 之后的 source-preview 实现状态。legacy 网络同步路径继续 hard-off。
